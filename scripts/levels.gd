...
func generate_chunk(chunk_pos):
	...
	var platform = chunk.get_node("platform");
	...
	for i in range(6):
		var pos2 = Vector3(rand_range(-8.0,8.0), 0, rand_range(-8.0,8.0));
		
		if rand_range(10, 100) > 50 && (!spawn_base || (spawn_base && !Rect2(-10,-10,10,10).has_point(Vector2(pos2.x, pos2.z)))):
